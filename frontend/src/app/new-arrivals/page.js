...
export default function NewArrivalsPage() {
  ...
  // Mock new arrivals (take first 8 available products after reverse)
  const newArrivals = products.filter(p => p.isAvailable).slice(0, 8);
    ...
    <div className="min-h-screen bg-white text-slate-900 selection:bg-yellow-200">
      ...
      <div className="bg-black py-16 text-center relative overflow-hidden">
        ...
        <div className="relative z-10 max-w-[800px] mx-auto px-4">
          <span className="text-yellow-500 font-bold tracking-widest text-xs uppercase mb-2 block">Latest Additions</span>
          <h1 className="text-4xl md:text-5xl font-black text-white tracking-widest uppercase mb-4">New Arrivals</h1>
          <p className="text-gray-300 text-lg font-light">Discover the newest trends in premium sanitary design.</p>
        </div>
      </div>
              ...
              return (
                <Link href={`/product/${product._id}`} key={product._id} className="group flex flex-col relative block cursor-pointer">
                  ...
                  <div className="absolute top-2 left-2 z-10 flex flex-col gap-1">
                    <span className="bg-black text-white text-[10px] font-bold px-2 py-0.5 uppercase tracking-wider">New In</span>
                  </div>
